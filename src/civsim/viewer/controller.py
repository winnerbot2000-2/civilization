...
class ViewerController:
    ...
    def _retune_step_budget(self) -> None:
        tuned_profiles = {(192, 24), (256, 32), (384, 48), (512, 64)}
        if (self.max_steps_per_advance, self.step_chunk_size) not in tuned_profiles:
            return
        living_agents = sum(1 for agent in self.state.agents if agent.alive)
        if living_agents >= 320:
            self.max_steps_per_advance = 192
            self.step_chunk_size = 24
        elif living_agents >= 200:
            self.max_steps_per_advance = 256
            self.step_chunk_size = 32
        elif living_agents >= 120:
            self.max_steps_per_advance = 384
            self.step_chunk_size = 48
        else:
            self.max_steps_per_advance = 512
            self.step_chunk_size = 64
