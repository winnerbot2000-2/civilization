...
from ..analysis.detectors import CampRecord, ClusterRecord, detect_camps, detect_clusters
...
@dataclass(slots=True)
class ViewerSnapshotCache:
    tick: int = -1
    selected_agent_id: int | None = None
    camps: list[CampRecord] = field(default_factory=list)
    clusters: list[ClusterRecord] = field(default_factory=list)
    metrics: MetricsSnapshot | None = None
    events: list[str] = field(default_factory=list)
    selected_lines: list[str] = field(default_factory=lambda: ["No agent selected.", "Click an agent marker on the map."])
...
def site_reuse_frequency(state, camps: list[CampRecord] | None = None) -> float:
    camps = detect_camps(state.world) if camps is None else camps
    if not camps:
        return 0.0
...
def build_metrics_snapshot(
    state,
    recent_event_count: int,
    camps: list[CampRecord] | None = None,
    clusters: list[ClusterRecord] | None = None,
) -> MetricsSnapshot:
    living = [agent for agent in state.agents if agent.alive]
    camps = detect_camps(state.world) if camps is None else camps
    clusters = detect_clusters(state) if clusters is None else clusters
    ...
    return MetricsSnapshot(
        ...
        site_reuse_frequency=site_reuse_frequency(state, camps),
...
def selected_agent_lines(state, agent_id: int | None, limit: int = 5) -> list[str]:
    ...
    mobility = age_mobility_factor(agent, state.config.life)
    work = age_work_factor(agent, state.config.life)
    lines = [
        ...
        f"Age days={agent.age_days}  mobility={mobility:.2f}  work={work:.2f}",
...
def build_viewer_snapshot(
    ...
    cache: ViewerSnapshotCache | None = None,
    *,
    event_limit: int = 14,
) -> tuple[ViewerFrameSnapshot, ViewerSnapshotCache]:
    cache = cache or ViewerSnapshotCache()
    if cache.tick != state.clock.tick or cache.metrics is None:
        cache.camps = detect_camps(state.world)
        cache.clusters = detect_clusters(state)
        cache.metrics = build_metrics_snapshot(
            state,
            recent_event_count=min(event_limit, len(state.event_bus.records)),
            camps=cache.camps,
            clusters=cache.clusters,
        )
        cache.events = recent_event_lines(state, limit=event_limit)
        cache.tick = state.clock.tick
        cache.selected_agent_id = None
    if cache.selected_agent_id != selected_agent_id:
        cache.selected_lines = selected_agent_lines(state, selected_agent_id)
        cache.selected_agent_id = selected_agent_id
    snapshot = ViewerFrameSnapshot(
        metrics=cache.metrics,
        camps=list(cache.camps),
        clusters=list(cache.clusters),
        events=list(cache.events),
        selected_lines=list(cache.selected_lines),
    )
