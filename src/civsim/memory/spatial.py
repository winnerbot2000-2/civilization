...
def remember_site(
    ...
    if entry is None:
        memory[key] = SpatialMemoryEntry(
            kind=kind,
            patch_id=patch_id,
            confidence=min(1.2, 0.4 + emotion * 0.18 + max(0.0, payoff) * 0.08),
            payoff=payoff,
            risk=risk,
            emotional_weight=emotion,
            revisit_bias=max(0.0, revisit_delta + max(0.0, payoff) * 0.2 + emotion * 0.12),
            avoidance_bias=max(0.0, avoidance_delta + max(0.0, risk) * 0.25 + emotion * 0.1),
            last_seen_day=day,
        )
    else:
        entry.confidence = min(1.5, entry.confidence * 0.82 + 0.16 + emotion * 0.1)
        entry.payoff = (entry.payoff * 0.68) + (payoff * 0.32)
        entry.risk = (entry.risk * 0.68) + (risk * 0.32)
        entry.emotional_weight = min(2.5, entry.emotional_weight * 0.78 + emotion * 0.55)
...
def decay_spatial_memory(memory: dict[tuple[str, int], SpatialMemoryEntry], decay: float) -> None:
    ...
    for key, entry in memory.items():
        entry.confidence *= 1.0 - decay
        entry.payoff *= 1.0 - (decay * 0.45)
        entry.risk *= 1.0 - (decay * 0.35)
        entry.emotional_weight *= 1.0 - (decay * 0.25)
        entry.revisit_bias *= 1.0 - (decay * 0.28)
        entry.avoidance_bias *= 1.0 - (decay * 0.18)
        if (
            entry.confidence <= 0.03
            and abs(entry.payoff) <= 0.03
            and entry.risk <= 0.03
            and entry.emotional_weight <= 0.03
            and abs(entry.revisit_bias) <= 0.03
