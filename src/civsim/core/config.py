...
class WorldConfig:
    ...
    water_source_count: int = 9
    danger_source_count: int = 5
    water_access_exponent: float = 1.55
    ...
    danger_fatigue_scale: float = 0.035
    danger_stress_scale: float = 0.12
...
class SocialConfig:
    ...
    caregiver_priority_bias: float = 0.9
...
class LifeConfig:
    ...
    elder_mobility_floor: float = 0.58
    elder_work_floor: float = 0.72
    gestation_days: int = 80
    fertility_cooldown_days: int = 120
    base_conception_chance: float = 0.02
    child_hunger_penalty: float = 0.002
...
def override_agent_counts(
    config: SimulationConfig,
    *,
    total_agents: int | None = None,
    initial_children: int | None = None,
) -> None:
    baseline_total = max(1, config.agents.initial_population + config.agents.initial_children)
    if total_agents is None:
        total_agents = baseline_total
    total_agents = max(1, int(total_agents))

    if initial_children is None:
        child_ratio = config.agents.initial_children / baseline_total
        initial_children = int(round(total_agents * child_ratio))
    initial_children = max(0, int(initial_children))
    initial_children = min(initial_children, max(0, total_agents - 1))

    config.agents.initial_children = initial_children
    config.agents.initial_population = max(1, total_agents - initial_children)
