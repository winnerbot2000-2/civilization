...
class SimulationState:
    ...
    def create_agent(
        ...
        return AgentState(
            ...
            alive=True,
            carried_food=0.2 if carried_food is None else carried_food,
...
def _seed_social_structure(state: SimulationState) -> None:
    ...
    caregiver_pool = [female for female in females if female.partner_id is not None]
    rng.shuffle(caregiver_pool)
    next_id = max(agent.agent_id for agent in state.agents) + 1
    for index in range(min(state.config.agents.initial_children, len(adults) // 2, len(caregiver_pool) * 2 if caregiver_pool else 0)):
        mother = caregiver_pool[index % len(caregiver_pool)]
        ...
        child = state.create_agent(
            ...
            age_days=rng.randint(max(40, (state.config.life.child_stage_days * 2) // 5), max(41, state.config.life.child_stage_days - 16)),
            caregiver_id=mother.agent_id,
            parent_ids=(mother.agent_id, father.agent_id),
            carried_food=max(0.8, state.config.agents.starting_food * 0.65),
        )
        child.age_stage = "child"
        child.hunger = rng.uniform(0.08, 0.18)
        child.thirst = rng.uniform(0.08, 0.18)
        child.fatigue = rng.uniform(0.02, 0.12)
        child.social_need = rng.uniform(0.08, 0.22)
        child.previous_hunger = child.hunger
        child.previous_thirst = child.thirst
        child.previous_fatigue = child.fatigue
        child.previous_social_need = child.social_need
...
def initialize_simulation(config: SimulationConfig, seed: int | None = None) -> SimulationState:
    ...
    for agent in state.agents:
        ...
        _seed_initial_spatial_memory(state, agent)
...
def run_tick(state: SimulationState) -> None:
    ...
    for agent in state.agents:
        ...
        _encode_perceived_water_memory(state, agent, percept)
...
def _encode_local_patch_memory(state: SimulationState, agent: AgentState) -> None:
    _remember_patch_features(state, agent, agent.patch_id)


def _remember_patch_features(state: SimulationState, agent: AgentState, patch_id: int, familiarity_scale: float = 1.0) -> None:
    if state.world.water[patch_id] > 0.25:
        remember_site(
            ...
            emotional_impact=agent.thirst * 0.12 * familiarity_scale,
            revisit_delta=max(0.0, float(state.world.water[patch_id]) - 0.35) * 0.05 * familiarity_scale,
        )
    if state.world.food[patch_id] > 0.3:
        remember_site(
            ...
            day=state.clock.day,
            max_entries=state.config.memory.max_spatial_entries,
            emotional_impact=agent.hunger * 0.1 * familiarity_scale,
            revisit_delta=max(0.0, float(state.world.food[patch_id]) - 0.3) * 0.04 * familiarity_scale,
        )
    ...
    if shelter_payoff > 0.25:
        remember_site(
            ...
            day=state.clock.day,
            max_entries=state.config.memory.max_spatial_entries,
            emotional_impact=(agent.fatigue + agent.stress) * 0.08 * familiarity_scale,
            revisit_delta=max(0.0, shelter_payoff - 0.25) * 0.05 * familiarity_scale,
        )
    if state.world.danger[patch_id] > 0.45:
        remember_site(
            ...
            day=state.clock.day,
            max_entries=state.config.memory.max_spatial_entries,
            emotional_impact=agent.stress * 0.15 * familiarity_scale,
            avoidance_delta=float(state.world.danger[patch_id]) * 0.08 * familiarity_scale,
        )


def _seed_initial_spatial_memory(state: SimulationState, agent: AgentState) -> None:
    visible_patches = state.world.grid.patches_within_radius(agent.patch_id, state.config.agents.perception_radius)
    local_patches = set(visible_patches)
    top_water = sorted(visible_patches, key=lambda patch_id: float(state.world.water[patch_id]), reverse=True)[:2]
    best_water = top_water[0] if top_water else agent.patch_id
    best_food = max(local_patches, key=lambda patch_id: float(state.world.food[patch_id]), default=agent.patch_id)
    best_shelter = max(local_patches, key=lambda patch_id: float(state.world.shelter[patch_id]), default=agent.patch_id)
    worst_danger = max(local_patches, key=lambda patch_id: float(state.world.danger[patch_id]), default=agent.patch_id)
    for patch_id in {best_water, best_food, best_shelter, worst_danger, agent.patch_id, *top_water}:
        familiarity = 0.55 if patch_id in top_water else 0.35
        _remember_patch_features(state, agent, patch_id, familiarity_scale=familiarity)


def _encode_perceived_water_memory(state: SimulationState, agent: AgentState, percept) -> None:
    if percept.best_visible_water_patch is None or percept.best_visible_water_value <= 0.18:
        return
    _remember_patch_features(
        state,
        agent,
        percept.best_visible_water_patch,
        familiarity_scale=0.22 + percept.best_visible_water_value * 0.08,
    )
