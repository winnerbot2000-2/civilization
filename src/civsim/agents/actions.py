...
def resolve_intents(state, intents, clock) -> dict[int, ActionOutcome]:
    ...
    for agent_id in sorted(intents):
        ...
        if not agent.alive:
            continue
        mobility_factor = age_mobility_factor(agent, state.config.life)
        work_factor = age_work_factor(agent, state.config.life)
        ...
        if intent.action == "drink":
            if world.water[agent.patch_id] > 0.2:
                ...
                remember_site(
                    ...
                    emotional_impact=start_thirst * 0.7 + relief * 0.55,
                    revisit_delta=relief * 1.2 + float(world.water[agent.patch_id]) * 0.12,
                )
        elif intent.action == "forage":
            available = float(world.food[agent.patch_id])
            if available > 0.02:
                gained = min(available, cfg_agents.base_food_yield * work_factor * (1.0 + agent.skills.foraging * 0.4))
            ...
            if target != agent.patch_id:
                ...
                move_fatigue = 0.026 * float(world.movement_cost[target]) / max(0.4, mobility_factor)
                if start_thirst > 0.6 and float(world.water[target]) > 0.18:
                    move_fatigue *= 0.72
                agent.fatigue += move_fatigue
            ...
            agent.fatigue -= cfg_agents.rest_recovery * shelter_factor * (0.88 + work_factor * 0.12)
            outcome.success = True
            ...
            agent.fatigue -= cfg_agents.rest_recovery * shelter_factor * (0.9 + work_factor * 0.1)
            ...
            if neighbors:
                ...
                old_patch = agent.patch_id
                agent.patch_id = target
                agent.fatigue += 0.032 * float(world.movement_cost[target]) / max(0.4, mobility_factor)
            ...
            if child is not None and child.patch_id == agent.patch_id:
                if agent.carried_food > 0.1:
                    ...
                    child.hunger -= amount * 0.45
                    child.thirst -= amount * 0.28
                child.social_need -= 0.2 * work_factor * (1.0 + agent.skills.caregiving * 0.5)
                child.stress -= 0.1
            ...
            agent.fatigue -= cfg_agents.rest_recovery * (0.95 + shelter_bonus * 1.1) * (0.9 + work_factor * 0.1)
        ...
        thirst_rate = cfg_agents.thirst_rate * (0.68 if agent.age_stage == "child" else 1.0)
        agent.thirst += thirst_rate + dry_penalty
