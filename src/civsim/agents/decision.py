...
from ..life.aging import age_mobility_factor, age_work_factor
...
def _local_patch_salience(agent, patch_id: int, world, profile: NeedProfile, percept, agents_by_id, social_config) -> float:
    ...
    return _local_patch_salience_with_biases(agent, patch_id, world, profile, social_config, social_bias, patch_memory_bias(agent.spatial_memory, patch_id))
...
def _focused_patches(agent, percept, world, profile: NeedProfile, salience_for_patch, rng) -> list[int]:
    if len(percept.nearby_patches) <= profile.attention_limit:
        return list(percept.nearby_patches)
    current_patch = agent.patch_id
    others = [patch_id for patch_id in percept.nearby_patches if patch_id != current_patch]
    scored = [(salience_for_patch(patch_id), patch_id) for patch_id in others]
    scored.sort(reverse=True)
    ranked = [patch_id for _, patch_id in scored]
...
def _thirst_pressure(profile: NeedProfile) -> float:
    return profile.urgency["thirst"] + profile.trend["thirst"] * 1.35


def _apply_hydration_preemption(
    *,
    agent,
    percept,
    profile: NeedProfile,
    candidates: list[ActionIntent],
    remembered_water_patch: int | None,
) -> None:
    pressure = _thirst_pressure(profile)
    if pressure < 0.82:
        return
    visible_water_patch = None
    if percept.best_visible_water_patch is not None and percept.best_visible_water_value > 0.18:
        visible_water_patch = percept.best_visible_water_patch
    water_here = percept.current_water > 0.18
    for candidate in candidates:
        if candidate.action == "drink":
            candidate.score += pressure * (1.15 + percept.current_water * 0.35)
            continue
        if candidate.action in {"move_local", "move_to_known_site"} and candidate.target_patch is not None:
            if candidate.target_patch == remembered_water_patch:
                candidate.score += pressure * 1.15
                continue
            if visible_water_patch is not None and candidate.target_patch == visible_water_patch:
                candidate.score += pressure * 0.95 + percept.best_visible_water_value * 0.3
                continue
        if candidate.action == "explore":
            candidate.score -= pressure * 1.25
        elif candidate.action in {"stay_with_kin", "share_food", "care_for_child", "rest", "wait"}:
            candidate.score -= pressure * (0.65 if water_here else 0.95)
        elif candidate.action == "forage":
            candidate.score -= pressure * 0.7
        elif candidate.action == "follow_caregiver" and water_here:
            candidate.score -= pressure * 0.45
...
def _add_candidate(
    ...
    *,
    ...
    mobility_factor = age_mobility_factor(agent, config.life)
    work_factor = age_work_factor(agent, config.life)
    if action in {"move_local", "move_to_known_site", "follow_caregiver", "avoid_danger", "explore"}:
        score -= (1.0 - mobility_factor) * 0.32
    elif action in {"forage", "care_for_child"}:
        score -= (1.0 - work_factor) * 0.14
    elif action in {"rest", "wait", "shelter_at_site"}:
        score += (1.0 - mobility_factor) * 0.12
...
def _fallback_intent(
    ...
    if _thirst_pressure(profile) > 0.95:
        if percept.current_water > 0.18:
            return ActionIntent(agent_id=agent.agent_id, action="drink", score=_thirst_pressure(profile) + percept.current_water)
        if percept.best_visible_water_patch is not None and percept.best_visible_water_patch != agent.patch_id and percept.best_visible_water_value > 0.18:
            return ActionIntent(
                agent_id=agent.agent_id,
                action="move_local",
                target_patch=percept.best_visible_water_patch,
                score=_thirst_pressure(profile) + percept.best_visible_water_value,
            )
        if remembered_water_patch is not None and remembered_water_patch != agent.patch_id:
...
def _best_social_patch(agent, percept, world, social_config, social_bias_for_patch) -> tuple[int | None, object | None]:
    best_patch = None
    best_bias = None
    best_score = 0.0
    for patch_id in percept.nearby_patches:
        ...
        if not occupants:
            continue
        bias = social_bias_for_patch(patch_id)
        score = (
            bias.affinity
...
def generate_action_intent(agent, percept, world, agents_by_id, clock, config, rng) -> ActionIntent:
    ...
    social_bias_cache: dict[int, object] = {}
    memory_bias_cache: dict[int, object] = {}
    salience_cache: dict[int, float] = {}

    def social_bias_for_patch(patch_id: int):
        bias = social_bias_cache.get(patch_id)
        if bias is None:
            bias = social_patch_bias(agent, world.occupancy.get(patch_id, ()), agents_by_id, config.social)
            social_bias_cache[patch_id] = bias
        return bias

    def memory_bias_for_patch(patch_id: int):
        bias = memory_bias_cache.get(patch_id)
        if bias is None:
            bias = patch_memory_bias(agent.spatial_memory, patch_id)
            memory_bias_cache[patch_id] = bias
        return bias

    def salience_for_patch(patch_id: int) -> float:
        score = salience_cache.get(patch_id)
        if score is None:
            score = _local_patch_salience_with_biases(
                agent,
                patch_id,
                world,
                profile,
                config.social,
                social_bias_for_patch(patch_id),
                memory_bias_for_patch(patch_id),
            )
            salience_cache[patch_id] = score
        return score

    remembered_water = select_best_site(agent.spatial_memory, "water", agent.patch_id, world.grid)
    if percept.current_water > 0.18 and profile.urgency["thirst"] > 0.92 and _thirst_pressure(profile) > 1.0 and percept.current_danger < 0.7:
        return ActionIntent(
            agent_id=agent.agent_id,
            action="drink",
            score=_thirst_pressure(profile) + percept.current_water * 1.2,
        )
    focused_patches = _focused_patches(agent, percept, world, profile, salience_for_patch, rng)
    candidates: list[ActionIntent] = []
    ...
    if percept.current_water > 0.2 and agent.thirst > 0.14:
        _add_candidate(
            candidates,
            agent=agent,
            config=config,
            ...
            profile=profile,
            action="drink",
            urgency=profile.urgency["thirst"] * 1.5,
            trend=profile.trend["thirst"] * 1.1,
            expected=percept.current_water * (0.2 + profile.urgency["thirst"] * 1.0)
            + current_water_expected * 0.35
            + current_patch_bias.revisit * 0.14,
            risk=percept.current_danger * 0.08 + current_patch_bias.avoidance * 0.08 + current_social_bias.avoidance * 0.06,
            context="water",
            uncertainty=current_water_uncertainty * 0.16,
        )
    ...
    if agent.age_stage != "child":
        ...
        if needy_children and (agent.carried_food > 0.12 or world.site_markers.get(agent.patch_id, None) is not None):
    ...
    if percept.best_visible_water_patch is not None and percept.best_visible_water_patch != agent.patch_id and percept.best_visible_water_value > 0.18 and agent.thirst > 0.28:
        visible_distance = world.grid.distance(agent.patch_id, percept.best_visible_water_patch)
        _add_candidate(
            candidates,
            agent=agent,
            config=config,
            ...
            profile=profile,
            action="move_local",
            urgency=profile.urgency["thirst"] * 1.3,
            trend=profile.trend["thirst"] * 1.1,
            expected=percept.best_visible_water_value * (0.35 + profile.urgency["thirst"] * 0.85) + agent.skills.navigation * 0.12,
            risk=world.danger[percept.best_visible_water_patch] * 0.12
            + world.movement_cost[percept.best_visible_water_patch] * 0.05
            + visible_distance * 0.03,
            target_patch=percept.best_visible_water_patch,
            context="water",
            uncertainty=max(0.0, 0.18 - percept.best_visible_water_value) * 0.2,
        )
    ...
    if agent.thirst > 0.26:
        if remembered_water is not None and remembered_water.patch_id != agent.patch_id:
            ...
            _add_candidate(
                ...
                urgency=profile.urgency["thirst"] * 1.85,
                trend=profile.trend["thirst"] * 1.15,
                expected=remembered_water.payoff * 0.4
                + remembered_water.confidence * 1.0
                + remembered_water.revisit_bias * 0.55
                + remembered_water.emotional_weight * 0.1,
                risk=distance * 0.025
    ...
    _add_candidate(
        ...
        risk=profile.fear * 0.28 + profile.urgency["hunger"] * 0.24 + profile.urgency["thirst"] * 0.55 + profile.urgency["fatigue"] * 0.16,
        uncertainty=explore_uncertainty * 0.15,
    )

    _apply_hydration_preemption(
        agent=agent,
        percept=percept,
        profile=profile,
    ...
    if critical_pressure > 0.9 and top.action in {
        ...
        return top
    if _thirst_pressure(profile) > 1.0 and top.action in {"drink", "move_to_known_site", "move_local"}:
        if top.action != "move_local" or (
            top.target_patch is not None
            and (
                (remembered_water is not None and top.target_patch == remembered_water.patch_id)
                or (percept.best_visible_water_patch is not None and top.target_patch == percept.best_visible_water_patch)
            )
        ):
            return top
